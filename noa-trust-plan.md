...
| **P1-12** | ⚪ **OWNER-DECISION** (opened 2026-08-04) — the genuine weakness both seats were circling, and it was already written down: **any keyring-trusted key can forge a checkpoint over any head** when no identity manifest is supplied. Plain words: *today, any key we already trust can vouch for a chain's endpoint.* Closing it changes a published format across FIVE independent verifier implementations plus vector regeneration — a KURAL 4 fork, not a bug fix. | **proof:** `grep -n "any keyring-trusted key can forge" docs/receipt-spec.md` → recorded verbatim as a residual needing the v1.0 external anchor | an owner decision at release level, or the v1.0 anchor lands |
