...
| **P1-3** | 🔴 **OPEN** · **R8-17 / F-1 / F-7b** — the injected DisplaySealer's output is signed with no re-check of tenant/holdId/deferredReceiptHash/expiresAt/recipients/aadHash. ⚠ **CORRECTED 2026-08-03:** ADR-0005 §8 has said **NOT CLOSED** since 07-31; what was live was the ADR CONTRADICTING ITSELF at `:24` and `:181` — fixed. Knockout G5 is **formally WITHDRAWN**, not missing: `lint-control-knockout.mjs:96-108` refuses to register a knockout for a control that was never built. | **proof:** `grep -rn "aad" packages/gate/src` → **1** hit (`types.ts`, an unused optional field) | the egress re-check exists **and** G5 is registered in the SAME commit, DETECTOR_TRIGGERED |
