...
| ~~**P1-2**~~ | ⬇ **4 of 5 CLOSED 2026-08-01 (`c84ae56`) — and the cause was the INSTRUMENT, not the controls.** The four index-walk entries declared `andAlso` (a paired mutation); the runner read that key **zero** times, so each applied HALF the mutation its own text describes and reported `DETECTOR_DID_NOT_TRIGGER`. We had been reading that as "not load-bearing". codex proposed deleting all four on exactly that reading; **the lead overruled it** — that conclusion came from an instrument we now knew had not performed the measurement. With `andAlso` implemented, all four are **proven load-bearing 1/1**. Deleting them would have discarded four working controls on the word of a broken tool. The class is closed too: an unknown registry key is now a HARD ERROR naming entry and key, not a silent skip. **Still open: `g4-render-node-single-input`** — measured clean 6/6 GREEN, mutated 6/6 GREEN, so its "must go red" claim is withdrawn in ADR-0005:252. The INVARIANT stands; what is missing is anything that measures it. | **[LEAD-VERIFIED, re-run independently]** bogus key → `Error: invalid knockout entry "zz-bogus-probe": unknown key "bogusKey"` exit 1, file restored byte-exact · all four entries `proven load-bearing 1/1` · typecheck:all 0 · lint:resolver-parity 0 · knockout selftest 0. Full 60-entry registry at `3af47d0` beforehand: 55 TRIGGERED / 4 DID_NOT_TRIGGER / 1 ANTI_VACUITY_FAILED — exactly the 5 tracked, no regression. | **MET for the four.** G4 remains: give it a test that genuinely measures the canonical-bytes invariant, or record with numbers why no reachable caller can. Do NOT force a RED — that is theatre |
