...
Generated by checking out git tag `v0.3.0`, running `npm ci && npm run build` there, then
running the generator script `scripts/gen-golden-vectors.mjs` against **that** build's
`dist/src/{builder,hash}.js` (see the script's header comment for the exact invocation). The
signing keys are fixed, hardcoded, TEST-ONLY Ed25519 keypairs (`golden-signer-1`,
`golden-signer-2`, `golden-signer-a`, `golden-signer-b`) — their private keys are intentionally
public in the generator script; never reuse them for anything real.
