...
- **The authority root can be moved out of this process, and the unsafe posture must be typed out.**
  The `execution-signer` key — the key that signs an Execution Grant — either lives in a separate
  process or the gate refuses to start. `NON-CLAIMS.md`'s authority-root corollary applies verbatim
  to the unsafe posture.

  ```bash
  # 1. Provision the signer key and the sidecar's own trust material (operator step, once).
  ...
  # prints: listening on … (kid=…)

  # 2. Point the gate at it. The signer's identity and the phone's enrolled public half are
  #    OPERATOR-supplied; the gate never learns either from the socket.
  export NOA_GATE_GRANT_SIGNER_SOCKET=/var/lib/noa/signer/grant.sock
  export NOA_GATE_GRANT_SIGNER_KID=…            NOA_GATE_GRANT_SIGNER_PUBLIC_KEY=…
  export NOA_GATE_APPROVER_KID=…                NOA_GATE_APPROVER_PUBLIC_KEY=…
  export NOA_GATE_APPROVER_HPKE_PUBLIC_KEY=…
  noa-gate serve

  # Development only, and it has to be said out loud:
  NOA_GATE_UNSAFE_IN_PROCESS_GRANT_KEY=1 noa-gate serve
  ```
  ...
  decision and ALLOWED receipt, refuses any grant whose `paramsHash` is not the one a human approved,
  and records spent approvals durably (`--replay-file`, defaulting beside the key file) so a restart
  cannot re-authorize one. `noa-gate serve` prints `grantKeyCustody` at boot so the posture is never
  a guess.
  ...
  with a shared group and a `0750` socket directory; the trust file must not be group-writable, and
  neither must that directory. Read `NON-CLAIMS.md`'s authority-root corollary for what is still not
  claimed.
