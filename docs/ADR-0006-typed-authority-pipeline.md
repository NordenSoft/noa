...
| **Anti-vacuity (knockout)** | **`display-aad-egress-check`** — ✅ **REGISTERED 2026-08-03**, `DETECTOR_TRIGGERED`, load-bearing 1/1. It was formally WITHDRAWN until that day, because its instruction is "delete the egress AAD verification" and the verification did not exist; the registry refused to manufacture the appearance of coverage over a control nobody had built, which is why the gap stayed visible. Registered in the SAME commit that built `verifySealedDisplayEgress`. | delete the G11 cross-check; must go RED |
