/**
 * INDEPENDENT RE-DERIVATION of codex QA finding F-5.
 *
 * CLAIM: `createRotatableSigner(...).keyring()` returns retired public keys with no temporal
 * information, and `verifyOutcomeReceipt` applies no retirement/activation check — so a holder of a
 * RETIRED private key can sign a NEW, post-retirement outcome receipt and it verifies clean.
 *
...
const rot = createRotatableSigner(old);
...
const attack = verifyOutcomeReceipt(forged, { keyring: rot.keyring() });
...
console.log(
  "\nVERDICT:",
  attack.ok
    ? "F-5 REPRODUCED — a retired signing identity produced a NEW outcome receipt that verifies clean"
    : "F-5 NOT REPRODUCED — the retired key was refused",
);
console.log(
  "controls:",
  r0.ok && r1.ok && !r2.ok
