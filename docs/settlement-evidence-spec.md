...
7. **Caps.** Each cap sets `chainStatus` (to `UNRECONFIRMED`) AND the code
   ...
   both roles) and the RAW-mode hold (envelope `mode != "ENFORCED"`, or a gate-signed hold
   resolution carrying `HUMAN_ACK_UNENFORCED`). Only `chainStatus: RECONFIRMED` may upgrade any
   outcome anywhere.
